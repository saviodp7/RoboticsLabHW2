...
int main(int argc, char **argv)
{
    ...
    trajectory_point p = planner.compute_circ_traj_cubic_prof(t);
    ...
    while ((ros::Time::now()-begin).toSec() < 2*traj_duration + init_time_slot)
    {
        if (robot_state_available)
        {
            ...
            {
                p = planner.compute_circ_traj_cubic_prof(0.0);
            }
            else if(t > init_time_slot && t <= traj_duration + init_time_slot)
            {
                p = planner.compute_circ_traj_cubic_prof(t-init_time_slot);
